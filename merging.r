...
merge_categories <- function(orders_enriched) {
  ...
  merge_categories <- c("furniture_bedroom", "furniture_mattress_and_upholstery", "kitchen_dining_laundry_garden_furniture", "la_cuisine", "home_comfort_2")
  orders_enriched$product_category_name_english[orders_enriched$product_category_name_english %in% merge_categories] <- "home_furniture"
  
  # Merge construction tool related categories
  merge_categories <- c("construction_tools_safety", "costruction_tools_garden", "costruction_tools_tools", "signaling_and_security", "security_and_services")
  orders_enriched$product_category_name_english[orders_enriched$product_category_name_english %in% merge_categories] <- "construction_tools"
  
  # Merge appliance related categories
  merge_categories <- c("home_appliances_2", "small_appliances_home_oven_and_coffee")
  orders_enriched$product_category_name_english[orders_enriched$product_category_name_english %in% merge_categories] <- "appliances"
  
  # Merge industry related categories
  merge_categories <- c("agro_industry_and_commerce", "industry_commerce_and_business")
  orders_enriched$product_category_name_english[orders_enriched$product_category_name_english %in% merge_categories] <- "industry_commerce"
